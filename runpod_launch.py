...
def done():
    return hf_has("runs/final/results.json") and hf_has("runs/final/ENGRAM_V6.md")
...
def mirror_final_to_branch(pid):
    ...
    for rp, dst in (("runs/final/results.json", "results.json"),
                    ("runs/final/ENGRAM_V6.md", "ENGRAM_V6.md"),
    ...
    for f in hf_tree("runs/final/engram"):
        ...
        if p.endswith((".json", ".png", ".csv")):
            try:
                hf_to_file(p, os.path.join(HERE, "results", "engram_v6",
                                           os.path.basename(p)))
                got.append(p)
            except Exception:
                pass
    if got:
        safe_push(f"engram-v6 results mirrored from HF (pod {pid})",
                  "results.json", "ENGRAM_V6.md", "status", "results")
    return got
